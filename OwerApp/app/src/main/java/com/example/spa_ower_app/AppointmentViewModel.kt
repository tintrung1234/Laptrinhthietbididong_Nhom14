...
class AppointmentViewModel: ViewModel() {
    ...
    // Phương thức để cập nhật state của Appointment theo appointmentID (documentID)
    fun updateAppointmentState(appointmentID: String, newState: Int) {
        val appointmentRef = db.collection("Appointments").document(appointmentID)

        // Cập nhật field State
        appointmentRef.update("State", newState)
            .addOnSuccessListener {
                Log.d("AppointmentViewModel", "Successfully updated the state")
                // Cập nhật lại danh sách appointments nếu cần thiết
                fetchAppointments()
            }
            .addOnFailureListener { exception ->
                Log.e("AppointmentViewModel", "Error updating state", exception)
            }
    }
}